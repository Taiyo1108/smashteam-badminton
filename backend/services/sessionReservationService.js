...
/**
 * Lấy chi tiết buổi tập kèm số liệu thống kê reservation và trạng thái của user hiện tại
 */
async function getSessionDetails(sessionId, userId = null) {
  ...
  // Đếm các số liệu trạng thái đặt chỗ
  const statsRes = await db.query(
    `SELECT 
      COUNT(*) FILTER (WHERE status = 'RESERVED') AS reserved_count,
      COUNT(*) FILTER (WHERE status = 'CONFIRMED') AS confirmed_count,
      COUNT(*) FILTER (WHERE status = 'CHECKED_IN' OR status = 'going') AS checked_in_count,
      COUNT(*) FILTER (WHERE status = 'CANCELLED') AS cancelled_count,
      COUNT(*) FILTER (WHERE status = 'NO_SHOW') AS no_show_count,
      COUNT(*) FILTER (WHERE status IN ('RESERVED', 'CONFIRMED', 'CHECKED_IN', 'going')) AS active_count,
  ...
  const activeCount = parseInt(stats.active_count, 10);
  ...
  const waitingCount = parseInt(waitlistStats.rows[0].waiting_count, 10);
  const offeredCount = parseInt(waitlistStats.rows[0].offered_count, 10);
  ...
  const checkinOpen = session.checkin_open_at ? new Date(session.checkin_open_at) : new Date(session.date_time.getTime() - 30 * 60000);
  const checkinClose = session.checkin_close_at ? new Date(session.checkin_close_at) : new Date(session.date_time.getTime() + 30 * 60000);
  ...
  return {
    ...
    isFull,
    stats: {
      reserved: parseInt(stats.reserved_count, 10),
      confirmed: parseInt(stats.confirmed_count, 10),
      checkedIn: parseInt(stats.checked_in_count, 10),
      cancelled: parseInt(stats.cancelled_count, 10),
      noShow: parseInt(stats.no_show_count, 10),
      active: activeCount,
      pendingLateCancel: parseInt(stats.pending_late_cancel_count, 10),
...
async function reserveSession(sessionId, userId) {
  ...
  try {
    ...
    // 3. Đếm số lượng slot active hiện tại
    const countRes = await client.query(
      `SELECT count(*) as active_count
       FROM attendances
       WHERE session_id = $1::uuid AND status IN ('RESERVED', 'CONFIRMED', 'CHECKED_IN', 'going');`,
      [sessionId]
    );
