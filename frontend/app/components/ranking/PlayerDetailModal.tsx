...
import { X, Trophy, Flame, TrendingUp, TrendingDown, Minus, Target, Award, Sparkles } from "lucide-react";
      ...
      >
        ...
        <div className="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          <div className="p-3.5 rounded-2xl bg-slate-900 text-white flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <span className="text-[10px] uppercase font-black tracking-wider text-slate-400">Mục tiêu thứ hạng</span>
          ...
          {myPlayer && !isMe && (
            <div className="p-3.5 rounded-2xl bg-indigo-50/70 border border-indigo-100 text-xs text-indigo-900">
              <div className="flex items-center justify-between font-bold mb-1">
                <span>So với bạn (Hạng #{myPlayer.rank}):</span>
              ...
              <p className="text-[11px] text-indigo-700/80">
                {eloDiffWithMe !== null && eloDiffWithMe > 0
                  ? `Cần tích lũy thêm khoảng ${eloDiffWithMe} ELO để bắt kịp vị trí của ${player.user.full_name}.`
                  : `Bạn đang có lợi thế thứ hạng trước đối thủ này trên bảng xếp hạng.`}
              </p>
            </div>
          )}
        </div>
